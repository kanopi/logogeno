...
CONFIG = {
    ...
    "Drupal AI": {
        "background_gradient_1": "#009CDE",
        "background_gradient_2": "#009CDE",
        "background_gradient_3": "#12285F",
        "background_color": "#12285F",
        "foreground_color": "#ffffff",
        "gradient_1": "0%",
        "gradient_2": "1%",
        "gradient_3": "100%",
        "svg_path": '''<svg width="300" height="300" viewBox="0 0 300 300" xmlns="http://www.w3.org/2000/svg"><path fill="#fff" d="M163.52 119.581c-.13-.259-.26-.389-.52-.648-12.723-14.26-30.769-34.486-43.751-48.876-4.025-4.279-8.05-8.557-11.815-12.965-.909-1.038-1.688-2.074-2.597-3.112-.779-.777-1.168-1.297-1.168-1.297h.13a61.8 61.8 0 0 1-10.386-17.242l-.65-1.426c0-.13-.13-.13-.259-.26-.13-.128-.39-.13-.519-.13h-.13c-.26 0-.39.13-.52.13l-.259.26-.649 1.426a68.5 68.5 0 0 1-10.386 17.242h.13s-.39.519-1.168 1.297c-.909 1.038-1.688 2.074-2.597 3.112A558 558 0 0 1 64.59 70.057c-12.853 14.39-31.03 34.614-43.752 48.876-.13.26-.26.39-.519.648-48.037 63.914 4.154 116.031 4.154 116.031h-.13c15.45 17.892 37.39 28.911 60.89 30.596q3.116.39 6.62.39h.131q3.31 0 6.62-.39c23.63-1.686 45.441-12.705 60.891-30.596h-.13s52.19-52.117 4.154-116.03M51.737 164.567l-1.168 1.814-.13.13c-8.829 10.631-14.022 22.04-15.71 33.319-.26 1.426-1.558 2.333-2.857 2.074-.908-.13-1.558-.777-1.948-1.556a64.8 64.8 0 0 1-4.674-16.984c-2.856-20.613 3.376-38.634 16.619-54.062l15.969-18.28c.908-1.037 2.466-1.166 3.505-.258l.26.259c4.284 4.926 9.867 11.279 16.1 18.54.778.907.778 2.333 0 3.37-8.57 10.242-17.657 21.132-25.837 31.503Zm73.743 53.413c-4.934 12.965-14.67 20.743-28.432 22.947-19.604 2.982-37.91-10.371-40.897-29.947 0-.26 0-.39-.13-.649-1.558-11.02 1.818-20.742 8.959-29.04 8.309-9.723 26.614-32.281 26.874-32.67.39.39 19.863 24.633 27.134 33.19 9.348 10.76 11.425 23.077 6.492 36.17m29.081-20.354c-.13.389-.39.907-.519 1.296-.519 1.297-1.947 1.946-3.245 1.426-.778-.389-1.428-1.037-1.558-1.945-1.817-10.89-7.01-21.78-15.45-32.023l-.13-.13-.779-1.166-.519-.649c-6.75-8.556-42.454-50.302-58.033-68.452-.779-.908-.779-2.333 0-3.37 5.193-5.705 10.386-11.41 15.45-17.244.909-1.037 2.467-1.037 3.506-.13l.13.13a187 187 0 0 1 6.751 7.52c14.151 15.815 28.432 31.373 42.195 47.32 17.267 19.965 21.292 43.042 12.204 67.415m99.07-5.832-4.661-15.208H225.68l-4.66 15.208h-14.606l22.578-64.135h16.566l22.667 64.135H253.62Zm-7.881-26.564-4.661-14.844a342 342 0 0 0-1.169-3.85c-.72-2.334-.96-3.139-1.427-4.746a99 99 0 0 1-1.169-4.2 94 94 0 0 1-1.259 4.472q-.7 2.507-1.427 5.004c-.415 1.556-.74 2.658-.987 3.307l-4.57 14.844h16.657Zm54.502 26.564h-29.108v-7.689l7.79-3.578v-41.33l-7.79-3.579v-7.688h29.108v7.688l-7.79 3.579v41.33l7.79 3.578Z"/></svg>'''
    }
}
...
def create_svg(repo_name, platform, output_path, no_platform_name=False):
    # Get CONFIG.
    ...
    if repo_name:
        ...
        # Calculate repo name Y position based on whether platform name is shown
        if no_platform_name:
            repo_y = text_y
        else:
            repo_y = text_y + platform_font_size + 8  # 8px vertical spacing
        # Print the Repo name.
        ET.SubElement(svg, 'text', {
            'x': '50%',
            'y': str(repo_y),
...
def generate_logo(repo_name, platform, no_platform_name=False):
    ...
    create_svg(repo_name, platform, svg_path, no_platform_name)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--no-platform-name", action="store_true", help="Exclude the platform name from the logo.")
    args = parser.parse_args()

    generate_logo(args.repo_name, args.platform, args.no_platform_name)
